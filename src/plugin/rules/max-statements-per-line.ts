...
type StatementNode = Node & Rule.NodeParentExtension;
...
const rule: Rule.RuleModule = {
   ...
   create(context) {
      ...
      let lastStatementLine = 0,
          ...
          firstExtraStatement: StatementNode | null = null;
       ...
       */
      function getActualLastToken(node: Node): ReturnType<typeof sourceCode.getLastToken> {
       ...
       * @returns {void}
       */
      function enterStatement(node: StatementNode): void {
          ...
          */
         const parent = node.parent as StatementNode & { alternate?: StatementNode };

         if (SINGLE_CHILD_ALLOWED.test(parent.type) && parent.alternate !== node) {
            return;
         }

         // Skip to allow expressions in single-line, block-style arrow functions
         const grandparent = parent.parent as StatementNode,
               isParentArrow = parent.type === 'BlockStatement' && grandparent?.type === 'ArrowFunctionExpression',
               parentFirstToken = sourceCode.getFirstToken(parent),
               parentLastToken = sourceCode.getLastToken(parent);

         const isParentOneLine = parentFirstToken && parentLastToken
            && astUtils.isTokenOnSameLine(parentFirstToken, parentLastToken);
       ...
       * @returns {void}
       */
      function leaveStatement(node: StatementNode): void {
         const token = getActualLastToken(node),
               line = token?.loc?.end.line ?? 0;
