...
import * as astUtils from '../helpers/ast-utils.js';
...
const rule: Rule.RuleModule = {
   ...
   create(context) {
       ...
       */
      function removeNewlineBetween(firstToken: Token | null, secondToken: Token | null): Rule.ReportFixer | null {
         ...
         return (fixer: Rule.RuleFixer) => { return fixer.replaceTextRange(textRange, ' '); };
      }
       ...
       * @returns {void}
       */
      function validateCurlyPair(openingCurly: Token | null, closingCurly: Token | null): void {
         ...
         const singleLineException = params.allowSingleLine && tokenAfterOpeningCurly
            && astUtils.isTokenOnSameLine(openingCurly, closingCurly);

         const isOpeningCurlyOnSameLine = tokenAfterOpeningCurly
            && astUtils.isTokenOnSameLine(openingCurly, tokenAfterOpeningCurly);

         const isClosingCurlyOnSameLine = tokenBeforeClosingCurly
            && astUtils.isTokenOnSameLine(tokenBeforeClosingCurly, closingCurly);

         const isBeforeOnDiffLine = tokenBeforeOpeningCurly
            && !astUtils.isTokenOnSameLine(tokenBeforeOpeningCurly, openingCurly);

         if (style !== 'allman' && isBeforeOnDiffLine) {
            context.report({
               node: openingCurly,
         ...
         const isBeforeOnSameLine = tokenBeforeOpeningCurly
            && astUtils.isTokenOnSameLine(tokenBeforeOpeningCurly, openingCurly);

         if (style === 'allman' && isBeforeOnSameLine && !singleLineException) {
            context.report({
               node: openingCurly,
       ...
       * @returns {void}
       */
      function validateCurlyBeforeKeyword(curlyToken: Token | null): void {
         if (!curlyToken) {
            return;
         }
         ...
         if (style === '1tbs' && keywordToken && !astUtils.isTokenOnSameLine(curlyToken, keywordToken)) {
            context.report({
               node: curlyToken,
      ...
      return {
         BlockStatement(node) {
            const firstToken = sourceCode.getFirstToken(node),
                  ...
                  isOneLine = firstToken && lastToken && astUtils.isTokenOnSameLine(firstToken, lastToken);
